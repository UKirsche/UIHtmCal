...
namespace Transmute.Wpf
{
    ...
    public class GraphicsDeviceControl : HwndHost
    {
        ...
        public bool RenderingPaused = false;
        ...
        public Vector2 MousePosition
        {
            get
            {
                return new Vector2((float)mouseState.Position.X, (float)mouseState.Position.Y);
            }
        }
        ...
        /// <summary>
        /// Invoked when no other message is present
        /// </summary>
        public event EventHandler<HwndUpdateEventArgs> UpdateXna;

        /// <summary>
        /// Invoked when the control receives a mouse wheel message
        /// </summary>
        ...
        void CompositionTarget_Rendering(object sender, EventArgs e)
        {
            ...
            if (isMouseCaptured && (int)mouseState.Position.X != capturedMouseX && (int)mouseState.Position.Y != capturedMouseY)
            {
                ...
                mouseState.Position = mouseState.PreviousPosition = new System.Windows.Point(capturedMouseClientX, capturedMouseClientY);
            }
            ...
            // Update Game Time
            UpdateGameTime();
            // Invoke the event to update this control
            if (UpdateXna != null) UpdateXna(this, new HwndUpdateEventArgs(gameTime));

            // Invoke the event to render this control
            if (RenderXna != null) RenderXna(this, new GraphicsDeviceEventArgs(_GraphicsDevice));

            // Present to the screen, but only use the visible area of the back buffer
            _GraphicsDevice.Present(viewport.Bounds, null, hWnd);
        }
        ...
        void XnaWindowHost_Loaded(object sender, RoutedEventArgs e)
        {
            ...
            if (graphicsService == null)
            {
                graphicsService = GraphicsDeviceService.AddRef(hWnd, (int)ActualWidth, (int)ActualHeight); //(int)System.Windows.SystemParameters.VirtualScreenWidth, (int)System.Windows.SystemParameters.VirtualScreenHeight);
                ...
                if (LoadContent != null) LoadContent(this, new GraphicsDeviceEventArgs(_GraphicsDevice));               
            }
        }
        ...
        {
            Current_Activated(this, EventArgs.Empty);
        }
        ...
        protected override IntPtr WndProc(IntPtr hwnd, int msg, IntPtr wParam, IntPtr lParam, ref bool handled)
        {
            ...
            switch (msg)
            {
                ...
                case NativeMethods.WM_MOUSEWHEEL:
                    if (!applicationHasFocus) break;

                    mouseState.PreviousPosition = mouseState.Position;
                    mouseState.Position = new System.Windows.Point(NativeMethods.GetXLParam((int)lParam), NativeMethods.GetYLParam((int)lParam));

                    var zDelta = NativeMethods.GetWheelDelta((int)wParam);
                    var hDelta = 0;

                    if (HwndMouseWheel != null) HwndMouseWheel(this, new HwndMouseEventArgs(mouseState, zDelta, hDelta));

                    break;
                case NativeMethods.WM_LBUTTONDOWN:
                    mouseState.LeftButton = MouseButtonState.Pressed;
                    if (HwndLButtonDown != null) HwndLButtonDown(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_LBUTTONUP:
                    mouseState.LeftButton = MouseButtonState.Released;
                    if (HwndLButtonUp != null) HwndLButtonUp(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_LBUTTONDBLCLK:
                    if (HwndLButtonDblClick != null) HwndLButtonDblClick(this, new HwndMouseEventArgs(mouseState, MouseButton.Left));
                    break;
                case NativeMethods.WM_RBUTTONDOWN:
                    mouseState.RightButton = MouseButtonState.Pressed;
                    if (HwndRButtonDown != null) HwndRButtonDown(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_RBUTTONUP:
                    mouseState.RightButton = MouseButtonState.Released;
                    if (HwndRButtonUp != null) HwndRButtonUp(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_RBUTTONDBLCLK:
                    if (HwndRButtonDblClick != null) HwndRButtonDblClick(this, new HwndMouseEventArgs(mouseState, MouseButton.Right));
                    break;
                case NativeMethods.WM_MBUTTONDOWN:
                    mouseState.MiddleButton = MouseButtonState.Pressed;
                    if (HwndMButtonDown != null) HwndMButtonDown(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_MBUTTONUP:
                    mouseState.MiddleButton = MouseButtonState.Released;
                    if (HwndMButtonUp != null) HwndMButtonUp(this, new HwndMouseEventArgs(mouseState));
                    break;
                case NativeMethods.WM_MBUTTONDBLCLK:
                    if (HwndMButtonDblClick != null) HwndMButtonDblClick(this, new HwndMouseEventArgs(mouseState, MouseButton.Middle));
                    break;
                case NativeMethods.WM_XBUTTONDOWN:
                    if (((int)wParam & NativeMethods.MK_XBUTTON1) != 0)
                    {
                        mouseState.X1Button = MouseButtonState.Pressed;
                        if (HwndX1ButtonDown != null) HwndX1ButtonDown(this, new HwndMouseEventArgs(mouseState));
                    }
                    else if (((int)wParam & NativeMethods.MK_XBUTTON2) != 0)
                    {
                        mouseState.X2Button = MouseButtonState.Pressed;
                        if (HwndX2ButtonDown != null) HwndX2ButtonDown(this, new HwndMouseEventArgs(mouseState));
                    }
                    break;
                case NativeMethods.WM_XBUTTONUP:
                    ...
                    {
                        mouseState.X1Button = MouseButtonState.Released;
                        if (HwndX1ButtonUp != null) HwndX1ButtonUp(this, new HwndMouseEventArgs(mouseState));
                    }
                    else if (((int)wParam & NativeMethods.MK_XBUTTON2) != 0)
                    {
                        mouseState.X2Button = MouseButtonState.Released;
                        if (HwndX2ButtonUp != null) HwndX2ButtonUp(this, new HwndMouseEventArgs(mouseState));
                    }
                    break;
                case NativeMethods.WM_XBUTTONDBLCLK:
                    if (((int)wParam & NativeMethods.MK_XBUTTON1) != 0)
                    {
                        if (HwndX1ButtonDblClick != null) HwndX1ButtonDblClick(this, new HwndMouseEventArgs(mouseState, MouseButton.XButton1));
                    }
                    else if (((int)wParam & NativeMethods.MK_XBUTTON2) != 0)
                    {
                        if (HwndX2ButtonDblClick != null) HwndX2ButtonDblClick(this, new HwndMouseEventArgs(mouseState, MouseButton.XButton2));
                    }
                    break;
                case NativeMethods.WM_MOUSEMOVE:
                    ...
                    {
                        if (HwndMouseWheel != null) HwndMouseWheel(this, new HwndMouseEventArgs(mouseState, NativeMethods.GetWheelDelta((int)wParam), 0));
                        if (HwndMouseMove != null) HwndMouseMove(this, new HwndMouseEventArgs(mouseState));
                    }
                ...
                case NativeMethods.WM_MOUSELEAVE:
                    ...
                    if (isMouseCaptured) break;

                    // Reset the state which releases all buttons and 
                    // marks the mouse as not being in the window.
                    ResetMouseState();

                    if (HwndMouseLeave != null) HwndMouseLeave(this, new HwndMouseEventArgs(mouseState));
                    break;      
